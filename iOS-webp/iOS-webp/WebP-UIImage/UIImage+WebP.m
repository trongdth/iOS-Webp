...
+ (UIImage *)imageWithWebPAtPath:(NSString *)filePath {
    BOOL at2XExists = [[NSFileManager defaultManager] fileExistsAtPath: filePath];
...
- (NSData *)newDataWebPWithQuality:(float)quality{
    int rc = WebPGetEncoderVersion();
    NSLog(@"WebP encoder version: %d", rc);
    ...
    }else ret_size = WebPEncodeBGRA(rawData, width, height, stride, quality, &output);
